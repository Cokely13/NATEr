...
export default function GoalsPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const goalsRes = await fetch(`/api/goals?date=${today}`);
      const goalData = await goalsRes.json();
      setGoals(goalData);
        ...
        {loading ? (
          ...
          <ul className="space-y-8">
            {goals.map((goal) => {
              ...
              const minutes = progress?.minutesCompleted || 0;
              const percent = (minutes / goal.targetMinutes) * 100;
              const isShared = goal.sharedWith && goal.sharedWith.length > 0;
              const sharedNames = isShared
                ? goal.sharedWith.map((sw) => sw.user.name).join(", ")
                : "";

              return (
                <li
                  key={goal.id}
                  className={`border-4 ${getBorderClass(goal)} ${
                    isShared ? "bg-yellow-50" : "bg-white"
